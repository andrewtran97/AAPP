...
**Phase ID:** B25
**Phase Name:** Policy Change Ledger Dual Control
**Phase Type:** policy governance
...
Require dual-control evidence before policy activation.

Business goal:
- Prevent silent or unilateral policy mutation.

Technical goal:
- Verify proposal digest, policy digests, approvals, downgrade detection, and ledger entry creation.
...
- Policy can become hidden risk.
- Policy changes need evidence and approval trace.

Without this phase:
- Policy can change without review.
- Downgrade may activate without trusted approval.
...
- No cloud approval workflow.
- No external policy engine adapter.
- No automatic policy mutation.

### Future Considerations

- Evidence data governance.
- Policy backend adapter after B27.
...
Production files:
- `aapp/policy_change_ledger.py`

Test files:
- `tests/test_policy_change_ledger.py`

Fixture files:
- `tests/fixtures/policy_change_ledger/*`

Documentation:
- `docs/phase-notes/B25_SCOPE.md`
...
- `policy.change.ledger.jsonl`
- `policy.change.verdict.json`
- `policy.change.report.md`
...
### Decision 1: Dual control

Chosen:
- Require two distinct valid approvals.

Rejected:
- Allow single approval.

Reason:
- Policy changes affect enforcement.
...
- Dual approval -> APPROVED.
- One approval -> rejected.
- Downgrade without required approval -> rejected.
...
|---|---:|---|
| Silent mutation | High | Require proposal digest and dual approval. |
| Approver gaming | High | Reject duplicate approvers. |
...
- Policy changes become ledgered.

Qualitative outcome:

- Reviewer sees why a policy version became active.
